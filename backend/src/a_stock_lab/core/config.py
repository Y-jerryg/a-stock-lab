...
class Settings(BaseSettings):
    ...
    openai_research_model: str = "gpt-5.6-sol"
    ...
    tail_radar_on_demand_research_enabled: bool = False
    ...
    tail_radar_intraday_workers: int = Field(default=4, ge=1, le=8)
    ...
    tail_radar_worker_poll_interval_seconds: float = Field(default=5, gt=0, le=60)
    tail_radar_worker_preflight_lead_seconds: int = Field(default=60, ge=10, le=900)
    tail_radar_worker_maximum_start_delay_seconds: int = Field(default=30, ge=0, le=120)
    tail_radar_worker_heartbeat_max_age_seconds: int = Field(default=30, ge=5, le=300)
    ...
    @property
    def tail_radar_worker_preflight_lead(self) -> timedelta:
        return timedelta(seconds=self.tail_radar_worker_preflight_lead_seconds)

    @property
    def tail_radar_worker_maximum_start_delay(self) -> timedelta:
        return timedelta(seconds=self.tail_radar_worker_maximum_start_delay_seconds)
